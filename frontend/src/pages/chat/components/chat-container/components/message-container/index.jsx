...
const MessageContainer = () => {
  ...
  const [imageUrl, setShowImageUrl] = useState(null);
  ...
  const checkIfImage = (filePath) => {
    const imageRegex =
      /\.(png|jpg|jpeg|gif|bmp|tiff|tif|svg|ico|heic|heif)$/i;
      ...
      {message.messageType === "text" && (
        <div
          className={`${message.sender !== selectedChatData._id
            ? "bg-[#8417ff] text-white/90 border-[#ffffff]/50"
      ...
      )}
      {
        ...
        <div
          className={`${message.sender._id === userInfo.id
            ? "bg-[#2a2b33] text-white/90 border-[#ffffff]/50"
            : "bg-[#2a2b33] text-white/80 border-[#ffffff]/20"
            } border inline-block p-4 rounded my-1 max-w-[50%] break-words
     `}>
          {
            checkIfImage(message.fileUrl) ?
              (
                <div className="cursor-pointer"
                  onClick={() => {
                    setShowImage(true)
                    setShowImageUrl(message.fileUrl)
                  }}
                >
                  <img
              ...
              <Avatar className="h-8 w-8 rounded-full overflow-hidden">
                {
                  message.sender.image && (
                    <AvatarImage src={`${HOST}/${message.sender.image}` || "https://github.com/shadcn.png"} className="object-cover w-full h-full bg-black" />
                  )}
